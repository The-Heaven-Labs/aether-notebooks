...
- **D1 — Native paging legend, right-docked.** One `buildLegend(config, colors)` helper in
  ...
  `top: title?40:8`, `bottom:8` (ECharts derives the box height from `top`/`bottom`; a fixed
  `height:'80%'` would make ECharts ignore `bottom` and overflow short widgets), themed page icons/text. No custom DOM
