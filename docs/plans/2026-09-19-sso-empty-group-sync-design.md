# SSO Group Sync: Authoritative Empty Claims + Prefix Stripping + Display Names
...
Three gaps make Aether group sync unusable for IdP-driven access management:
...
3. **No human-friendly label.** IdP group names are frequently slugs (`aether-notebooks-data-analysts-infra`); prefix stripping alone cannot produce `Data Analysts Infra`, and guessing a de-slug transform mangles tokens (`dwh` → `Dwh`). There is no way for an admin to attach a display label that survives sync.

Because an absent claim is indistinguishable from a removed/misconfigured mapper, the two sync behaviors are **opt-in** provider settings so existing providers keep current semantics. Display labels (gap 3) are additive: they default to `NULL`, are rendered as a fallback, and change no sync behavior.
...
3. Let an admin attach a human-friendly display label to any group, rendered in the UI without ever participating in sync identity or matching.
4. Preserve all existing behavior for providers that do not opt in.
5. Avoid wiping memberships on transient IdP/UserInfo failures.

## Non-Goals

- `external_id` columns or IdP rename tracking (names remain the sync identity; an IdP rename still creates a new group row and drops the old membership)
- Automatic label derivation beyond literal prefix strip (no de-slug/title-casing heuristics — labels are admin-owned)
...
> Migration numbers assume `V114` is the highest shipped migration; renumber to
> the next free files when implementing if later migrations have landed.

### 1. Provider Configuration (migration V115)
...
### 4. Group Display Names (migration V116)

`groups.name` stays the sync identity (the resolved claim name). A nullable
`display_name` column carries an admin-owned label that the UI renders instead:
...
- **Identity vs label.** Matching, stale comparison, uniqueness
  (`UNIQUE (org_id, name)`), permission resolution, and audit resource names all
  keep using `name`. `display_name` never participates in any lookup.
- **Rendering.** Every UI surface renders `display_name?.trim() || name` through
  one shared helper (e.g. `web/src/utils/groupLabel.ts`). `NULL` or blank falls
  back to `name`.
- **Sync never writes it.** `FindOrCreateGroup` inserts rows with
  `display_name = NULL` and never updates it, so an IdP rename (which creates a
  new group row) cannot clobber an existing label, and repeated logins cannot
  overwrite an admin edit. Because identity is still `name`, a label attached to
  a stale group is retained with the group (groups are never deleted).
- **API.** `models.Group` gains `DisplayName *string` (`json:"display_name"`).
  `POST /groups` accepts an optional `display_name`; `PUT /groups/{id}` accepts
  `name` and/or `display_name`, where an omitted `name` keeps the current name,
  an omitted `display_name` keeps the label, and an explicit blank (`""` or
  whitespace) clears it to `NULL`. Values are trimmed. The canonical `Everyone`
  group rejects a label (400) — it is always rendered as "Everyone". (JSON
  `null` is indistinguishable from an omitted field with a plain `*string`
  decode; clients clear with `""`.)
- **UI.** `web/src/pages/GroupsPage.tsx` shows the label in the list and offers a
  "Display name" input in the create and rename dialogs (placeholder = the
  group's name). Subject pickers and grant tables
  (`PermissionsPanel`, `WarehouseTableGrants`, `NewTablesInbox`) render through
  the same helper.
- **Example.** IdP name `aether-notebooks-data-analysts-infra` with
  `strip_group_prefix = "aether-notebooks-"` stores `data-analysts-infra`; the
  admin sets the display name to `Data Analysts Infra`.

This deliberately does not track IdP renames: with names as identity, a renamed
IdP group becomes a new Aether group (fresh membership, no label) while the old
group keeps its label and members until manually cleaned up. Full `external_id`
identity is the follow-up that would fix both, at the cost of a larger
migration.
...
- `web/src/types/index.ts`: extend `SSOProvider`; extend `Group` with `display_name: string | null`.
...
- `group_handlers.go`: list/create/update read and write `display_name` (see §4).
- `GroupsPage.tsx` + shared `groupLabel` helper: label in list and dialogs; all other group-name render sites switch to the helper.
...
- Display names: create with/without label, update set/clear/keep-name-only, blank trims to `NULL`, `Everyone` rejects a label, `FindOrCreateGroup` leaves an existing `display_name` untouched, and list endpoints return it.
- Frontend: `groupLabel` unit tests (label, blank label, `NULL`) and a `GroupsPage` test for the create/edit dialog.
- Default-off paths keep existing tests green.

## Migration Plan

1. Migration V115 + `sso.Provider`/CRUD/serialization updates.
...
4. Callback wiring.
5. Migration V116 `groups.display_name` + group model/API + `FindOrCreateGroup` preservation.
6. Frontend: provider forms, `Group` type, `groupLabel` helper, `GroupsPage` dialogs, render sites + Swagger.
7. Tests and `docs/SSO_GROUP_PROVISIONING.md` updates.
